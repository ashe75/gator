...
const getUserNameCreatedFeed = `-- name: GetUserNameCreatedFeed :many
...
type GetUserNameCreatedFeedRow struct {
	Name   string
	Url    string
	Name_2 string
}

func (q *Queries) GetUserNameCreatedFeed(ctx context.Context) ([]GetUserNameCreatedFeedRow, error) {
	rows, err := q.db.QueryContext(ctx, getUserNameCreatedFeed)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetUserNameCreatedFeedRow
	for rows.Next() {
		var i GetUserNameCreatedFeedRow
